...
makeTheParser
  ...
  -> Bool                             -- ^ Whether the phoneme set supports casing.
...
makeTheParser phoneMap aspectMap phoneName mkMaj mkMin canBeCased wordDataNames aspectFuncs traitFuncs groupFuncs pps pops
  = runQS (poNameSuffix pops) $ makeTheParser' phoneMap aspectMap phoneName mkMaj mkMin canBeCased wordDataNames aspectFuncs traitFuncs groupFuncs pps pops
...
makeTheParser' phoneMap aspectMap phoneName mkMaj mkMin canBeCased wordDataNames aspectFuncs traitFuncs groupFuncs pps pops = do
  ...
  let classDictX = ppsClassDictionary pps
      ...
      autoStateMods = forMap (M.assocs autoStateDict) $ \(stName, stType) ->
        case stType of
          Nothing  -> ModifyStateBB stName False
          (Just _) -> ModifyStateVX stName
  ...
  let encPhoneType = if canBeCased
        then (AppT (ConT ''C2.CasedValue) (ConT phoneName))
        else (ConT phoneName)
  ...
  let spi = StaticParserInfo
              ...
              , spiEncPhoneType    = encPhoneType
              , spiResetContFunc   = resetContName
              , spiStateDictionary = newStateDict
              , spiWordTypeNames   = wordDataNames
              , spiResetStates     = autoStateMods
              , spiAspectFuncs     = aspectFuncs
              , spiTraitFuncs      = traitFuncs
              , spiGroupFuncs      = groupFuncs
              }
...
createCasesForStart grdName stateValName spi cp trieAnn@(TrieAnn _) _mph = case cp of
  ...
  (FollowPat _) -> Left ["Shouldn't encounter a \"FollowPat\" when using this function."]
...
createCasesForStart grdName stateValName spi cp TrieLeaf mph = case cp of
  ...
  (CharClass chks cn) -> do
    ...
    let checkExp = AppE (VarE classFunc) (VarE grdName)
        -- boolExp  = AppE (VarE 'isUpperCase) (VarE grdName)
        -- These lines were commented out, and I don't know why.
        -- I've uncommented them, since I'm pretty sure they're necessary.
        checkExp' = case mgrdExp of
          Nothing -> checkExp
          (Just guardExp) -> andE checkExp guardExp
...
data StaticParserInfo = StaticParserInfo
  ...
  -- | The `Type` used for phonemes. It can be either a plain
  --   phoneme, or one enclosed in `CasedValue`.
  , spiEncPhoneType   :: Type
  ...
  -- | A `M.Map` from `String`s to function `Name`s
  --   for checking membership in the aspect. Be
  --   careful of this one; the values slotted in
  --   here may be incorrect.
  , spiAspectFuncs :: M.Map String (Name, (Name, M.Map String Name))      
  -- | A `M.Map` from `String`s to `Names` of functions 
  --   that check whether a phoneme has a certain trait.
  --   Also contains `Name`s of constructors of the
  --   values of that trait, if it's a value trait.
  , spiTraitFuncs  :: M.Map String (Name, Maybe (Name, M.Map String Name)) 
  -- | A `M.Map` from `String`s to `Name`s of functions
  --   that check membership in a certain group.
  , spiGroupFuncs  :: M.Map String Name
  -- | The name of the function/action that resets
  --   the auto-states to off and then runs the
  --   next procedure
  , spiResetContFunc   :: Name
  -- | The "auto-off" state operations that are
  --   performed at the end of every phoneme, unless
  --   set to be turned (back) on.
  , spiResetStates     :: [ModifyStateX]
  }
...
instance Eq StaticParserInfo where
  x == y
    = ((spiAnnotationMap x) == (spiAnnotationMap y))
      ...
      && ((spiEncPhoneType x) == (spiEncPhoneType y))
...
instance Show StaticParserInfo where
  show x =
    "StaticParserInfo {spiAnnotationMap = " <> show (spiAnnotationMap x)
      ...
      <> ", spiEncPhoneType = "    <> show (spiEncPhoneType x)  
...
constructFunctionsBoth spi bl trie funNomStrt funNomRst = do
  ...
  let funcNames = (funNom1, funNom2)

  let ((trieS, trieNS), trie2) = splitTrie trie
  -- Construct the 
  eRslt1 <- constructFunctionsSB bl spi [WordStart] (S.empty, M.empty) funcNames trieS
  case eRslt1 of
    (Left errs) -> return $ Left errs
    (Right (decs1, st1, nom1)) -> do
      eRslt2 <- constructFunctionsSB bl spi [NotStart] st1 funcNames trieNS
      case eRslt2 of
        (Left errs) -> return $ Left errs
        (Right (decs2, st2, nom2)) -> do
          eRslt3 <- constructFunctionsSB bl spi [] st2 funcNames trie2
          case eRslt3 of
            (Left errs) -> return $ Left errs
...
constructFunctionsPat spi cps stVals funcNames trie
  = fmap getIt <$> constructFunctionsS spi cps stVals funcNames trie
...
constructFunctions
  ...
  -> TM.TMap CharPattern (TrieAnnotation, Maybe (PhoneResult, Caseness))
  -> QS (Either [String] ([Dec], Name))
constructFunctions spi funcNames trie
  = fmap getIt <$> constructFunctionsS spi [] (S.empty, M.empty) funcNames trie
  where getIt (decs,_,nom) = (decs,nom)
...
constructFunctionsB
  ...
  -> TM.TMap CharPattern (TrieAnnotation, Maybe (PhoneResult, Caseness))
  -> QS (Either [String] ([Dec], Name))
constructFunctionsB bl spi funcNames trie
  = fmap getIt <$> (constructFunctionsSB bl) spi [] (S.empty, M.empty) funcNames trie
  where getIt (decs,_,nom) = (decs,nom)
...
constructFunctionsSB
  ...
  -> TM.TMap CharPattern (TrieAnnotation, Maybe (PhoneResult, Caseness))
  -> QS (Either [String] ([Dec],(S.Set TrieAnnotation, M.Map TrieAnnotation Bool), Name))
constructFunctionsSB toReduce spi xcp stVals funcNames trie = do
  ...
  case (liftEitherList rslts) of
    ...
    (Right rsltsX) -> do
      ...
      (moreDecs, stt, errs) <- runRWST' () stVals $ forM subTriesX $ \((cp, ((tann, mval), subTrie )),thisbl) -> do
        ...
        constructFunctions' blName peekName thisbl spi subTrie (xcp ++ [cp]) tann mval funcNames
      ...
      moreDecs <- for conts $ \( tval@(nextAnn, nextVal) , nextCased, nextPat, nextTrie ) -> do
        ...
        xrslts <- if (nextAnn `S.member` curMemSet)
          then (return [])
            ...
            -- hmm...
            constructFunctions' blName peekName nextCased spi nextTrie (cPats ++ [nextPat]) nextAnn nextVal funcNames
        return xrslts
...
makeGuards
  ...
  -> [ModifyStateX]                -- ^ Auto-off State changes
...
phonemeRetZB mkMaj mkMin cs peekName rslts
  ...
  | cs == CMin = AppE ret $ formMulExp $ forMap rslts $ \rslt -> (mkMin rslt)
  -- Temp?
  | otherwise  = condCaseExp (ConE 'False) mkMaj mkMin rslts 
