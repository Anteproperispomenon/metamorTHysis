...
Description : Helper for MatcherT
...
for MatcherT. Since the same type can be
used for both the "regular" and "undo"
variants, it is defined in a separate 
module here.
...
--
--   plainReturn :: r -> MatchReturn m i v r
--   plainReturn x = PlainReturn $ \_ -> x
--
...
--   @   
--   
--   This could then be run with `Metamorth.ForOutput.Monad.Matcher.matches`
...
instance (Functor m) => Functor (MatchReturn m i v) where
